  ...
  if (err != CL_SUCCESS)
    {
      ...
      int status = clGetProgramBuildInfo(cv.main_program, cv.device_id, CL_PROGRAM_BUILD_LOG, 
                            sizeof(buffer), buffer, &len);
      #ifdef DEBUG
        printf("Status codes: %d %d %d %d\n", CL_BUILD_NONE, CL_BUILD_ERROR, CL_BUILD_SUCCESS, CL_BUILD_IN_PROGRESS);
        if (status == CL_BUILD_ERROR){printf("CL_BUILD_ERROR\n");}
        else if (status == CL_BUILD_NONE){printf("CL_BUILD_NONE\n");}
        else{printf("%d\n", status);}
      #endif
